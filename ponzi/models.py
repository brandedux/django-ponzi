...
class RewardPeriod(models.Model):
    ...
    def get_received(self):
        return sum([tx.amount for tx in self.tx_set.filter(amount__gt=0, donation=False, confirmed=True, isreward=False)])
    def get_sent(self):
        return sum([tx.amount for tx in self.tx_set.filter(amount__lt=0, donation=False)])
    def do_rewards(self):
        for t in  self.tx_set.filter(confirmed=True, rewarded=False, donation=False, isreward=False):
            self.reward_next()
    def reward_next(self):
        tx = self.tx_set.filter(confirmed=True, rewarded=False, donation=False, isreward=False).order_by('date')[0]
        if tx.get_reward_threshold()+FEE_BUFFER <= self.get_total():
            server = get_server()
            print "rewarding", tx.pk, tx.amount, tx.get_reward(), self.get_total(), server.getbalance()
            ...
            rewardtx, created = Tx.objects.get_or_create(txid=txid, addresspair=tx.addresspair, amount=rewardtx_amount)
